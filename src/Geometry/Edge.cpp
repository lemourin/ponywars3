...
bool Edge::colid(const Circle& circle) const {
    ...
    if (colidPoints(circle, res))
        if (insideAABB(res.first) || insideAABB(res.second))
            return true;

    return false;
}

bool Edge::colidPoints(const Circle& circle, std::pair<Vector2d, Vector2d>& res) const {
    double _x = x0-circle.x, _y = y0-circle.y, r = circle.r;

    double a = ((double)x*x+(double)y*y), b = 2.f*(_x*x+_y*y), c = _x*_x+_y*_y-r*r;
...
bool Edge::colid(const Edge &a) const {
    ...
    float d1 = Geometry::det3(p3, p4, p1),
            d2 = Geometry::det3(p3, p4, p2),
            d3 = Geometry::det3(p1, p2, p3),
            d4 = Geometry::det3(p1, p2, p4);

    if (d1*d2 < 0 && d3*d4 < 0)
        return true;
    if (fabs(d1) < EPS && a.insideAABB(p1))
		return true;
    if (fabs(d2) < EPS && a.insideAABB(p2))
		return true;
    if (fabs(d3) < EPS && insideAABB(p3))
		return true;
    if (fabs(d4) < EPS && insideAABB(p4))
		return true;
	return false;
}
...
bool Edge::insideAABB(const Vector2d& a) const {
    return insideAABB(a.x, a.y);
}
...
Edge Edge::operator-() const {
    return Edge(x0+x, y0+y, x0, y0);
}

Vector2d Edge::vector(float ang, float len) const {
    Vector2d res = points().second+Geometry::normalizedToLength(Vector2d(x, y), len);
