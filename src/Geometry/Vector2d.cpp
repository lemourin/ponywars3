...
Vector2d::Vector2d(const std::pair<float, float>& a): x(a.first), y(a.second) {
}
...
Vector2d& Vector2d::translate(const Vector2d &p) {
...
Vector2d::Direction Vector2d::direction(const Vector2d &a) const {
    if (fabs(Geometry::crossProduct(*this, a)) < 1e-9)
        return Straight;

    if (Geometry::crossProduct(*this, a) > 0)
        return Right;
    return Left;
}
