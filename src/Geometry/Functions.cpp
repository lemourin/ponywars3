...
bool Geometry::isLower(const Vector2d& low, const Vector2d& high) {
    if (fabs(low.y-high.y) < EPS && low.x < high.x)
        return true;
    return low.y < high.y;
}
...
Geometry::CmpAngleSort::CmpAngleSort(const Vector2d &mid):
    m_lowest(mid) {
}
...
void Geometry::convexHull(std::vector<Vector2d> &verts) {
    ...
    for (size_t i=0; i<verts.size(); i++) {
        while (res.size() >= 2 && det3(res[res.size()-2], res[res.size()-1], verts[i]) < EPS)
			res.pop_back();
...
bool Geometry::inRange(float a, float b1, float b2) {
    if (fabs(a-b1) < 1e-9 || fabs(a-b2) < 1e-9)
        return true;
    return a >= std::min(b1, b2) && a <= std::max(b1, b2);
}

bool Geometry::collision1d(float a1, float a2, float b1, float b2) {
    return Geometry::inRange(a1, b1, b2) ||
           Geometry::inRange(a2, b1, b2) ||
           Geometry::inRange(b1, a1, a2) ||
           Geometry::inRange(b2, a1, a2);
}

std::vector<Vector2d> Geometry::simplifyPolygon(std::vector<Vector2d> polygon, float eps) {
    if (polygon.size() < 3)
        return polygon;
    ...
    size_t index = std::max_element(polygon.begin()+1, polygon.end()-1, cmpDist)-polygon.begin();

    if (line.distance(polygon[index]) > eps) {
        std::vector<Vector2d> part1, part2, res;
        for (size_t i=0; i<=index; i++)
            part1.push_back(polygon[i]);
